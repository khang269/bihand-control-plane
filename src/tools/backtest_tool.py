...
def run_backtest(run_dir: str) -> str:
    ...
    # Deliberate deviation from upstream (300s): our Cloud Run Job timeout is
    # 600s total for the whole agent loop (skill load, file writes, backtest,
    # possible repair) — 300s here left too little headroom for everything
    # around the subprocess call itself, and a timeout here raises
    # subprocess.TimeoutExpired uncaught through run_backtest().
    runner = Runner(timeout=420)
    try:
    ...
    except Exception as exc:  # noqa: BLE001 — deliberate deviation from upstream:
        # a bare subprocess.run(timeout=...) raises TimeoutExpired uncaught;
        # left unhandled it propagated as an opaque registry-level exception
        # with none of the run's own stdout/stderr, making a real timeout
        # indistinguishable from every other failure. Surface it plainly.
        emit_progress("finalize", message="backtest subprocess failed")
        return json.dumps({
            "status": "error",
            "exit_code": None,
            "stdout": "",
